...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
	...
	vector<Store*> point2Store;
	for (int i = 0; i < auxStores.size(); i++) {
		Store* temp = &auxStores[i];
		point2Store.push_back(temp);
	}

	// TODO:  Sort the vector of pointers using the STL sort algorithm
	//        with comp as the ordering relationship.

	for (int unsorted = 1; unsorted < point2Store.size(); unsorted++)
	{
		Store* nextItem = point2Store[unsorted];
		int loc = unsorted;
		while (loc > 0 && comp(nextItem, point2Store[loc - 1]))
		{
			point2Store[loc] = point2Store[loc - 1];
			loc--;
		}
		point2Store[loc] = nextItem;
	}
