...
int countContains(const string a1[], int n1, const string a2[], int n2)
{
	...
	if (a1[0] != a2[0]) {
		b += countContains(a1 + 1, n1 - 1, a2, n2);
	}

	if (a1[0] == a2[0]) {

		b += countContains(a1 + 1, n1 - 1, a2 + 1, n2 - 1);
		b += countContains(a1 + 1, n1 - 1, a2, n2);
	}
...
void order(string a[], int n)
{
	...
	//	cout << a[n / 2] << endl << endl;
		divide(a, n, a[(n / 2)], i, j);
		//cout << a[i] << endl << a[j] << endl << endl;
		order(a, i);
		order(a+j, n - j);
	}
}
...
/*int main()
{
	//string a1[5] = { "1","1","1","2","5"};
	//string a2[3] = { "1","2","5"};

	//int a = countContains(a1, 5, a2, 3);

	string a[7] = { "5","3","2","1","4","7","8" };
